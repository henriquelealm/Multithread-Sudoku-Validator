...
void *verificaLinhaEColuna(void *ptr);
int min(int a, int b);
...
int main(int argc, char *argv[]) {
    ...
    if ((strchr(l1, ' ') != NULL) || (strchr(l2, ' ') != NULL)){ //verifica se as dimensões estão formatadas
    ...
    if(linhas != colunas){
        fprintf(stderr, "File out of format"); //verifica se é uma matriz quadrada
        exit(EXIT_FAILURE);
    }

    if(((sub_linhas*sub_colunas) != linhas) || ((sub_linhas*sub_colunas) != colunas)){  //verifica se o tamanho do subgrid é valido
        fprintf(stderr, "File out of format");
        exit(EXIT_FAILURE);
    }

    ptrPos = ftell(f);
    int countN = 0,num,countE = 0,countL=1;
    char elemento;
    
    while (fscanf(f, "%d", &num) == 1) { //verificar se todos os valores pertencem ao intervalo
        if(num > linhas || num < 1){
            fprintf(stderr, "File out of format");
            exit(EXIT_FAILURE);
        }
    ...
    while ((elemento = fgetc(f)) != EOF) { //conta os espaços e linhas
        if (elemento == ' ') {
            countE++;
        }
        else if (elemento == '\n') {
    ...
    if(countE != (linhas*colunas)-linhas){
        fprintf(stderr, "File out of format");
        exit(EXIT_FAILURE);
    }
    ...
    for (int i = 0; i < linhas; i++) {
        ...
        int tlinha = pthread_create(&threads[i], NULL, verificaLinhaEColuna, (void *)&ds[i]);
...
void *verificaSubgrid(void *ptr) {
    ...
    for (int i = sg->linhaSubgrid; i < sg->linhaSubgrid + sg->nLinhas; i++) {
        for (int j = sg->colunaSubgrid; j < sg->colunaSubgrid + sg->nColunas; j++) {
           // if (i < 0 || i >= N || j < 0 || j >= N) continue; segmentation fault
